...
EMBEDDINGS_JSONL, MENU_EMBEDDINGS_JSONL, REVIEW_EMBEDDINGS_JSONL
    Canonical CLIP vector files generated by the embedding pipelines.
"""
import json
import re
...
def _resolve_embedding_path(filename: str) -> Path:
    """
    Resolve a canonical embeddings file path under ``data/embeddings/``.
    """
    return DATA_DIR / "embeddings" / filename
...
_CURRENCY_SYMBOLS = "$€£¥"
_MOJIBAKE_MARKERS = ("�", "鈥", "Â", "â", "ã", "猸", "бд", "路")


def _format_menu_price(value: Any) -> str:
    """
    Return a display-safe menu price, or an empty string when the field is not a price.

    Menu prices are mostly stored as bare numbers, with a few slash-separated
    options or add-on markers. This helper keeps price display consistent and
    prevents prose or mojibake from being rendered as a price badge.
    """
    price = _clean_text(value)
    if not price:
        return ""

    if any(marker in price for marker in _MOJIBAKE_MARKERS):
        return ""
    ...
    if any(symbol in price for symbol in _CURRENCY_SYMBOLS):
        return price

    def is_inside_parentheses(index: int) -> bool:
        return price.rfind("(", 0, index) > price.rfind(")", 0, index)

    def add_dollar(match: re.Match[str]) -> str:
        token = match.group(0)
        if is_inside_parentheses(match.start()):
            return token
        if token.startswith("+"):
            return f"+${token[1:]}"
        return f"${token}"

    return re.sub(r"(?<![\w$€£¥])\+?\d+(?:[.,]\d+)?", add_dollar, price)
...
def _extract_menu_items(menu_rows: List[Dict[str, Any]], limit: int = 6) -> List[str]:
    ...
    for row in menu_rows[:limit]:
        ...
        price = _format_menu_price(row.get("price"))
        parts = [part for part in [dish, ingredients, price] if part]
        if parts:
